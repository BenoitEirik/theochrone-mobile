<script lang="ts" setup>
import { App } from '@capacitor/app'
import { Browser } from '@capacitor/browser'

const navStore = useNavStore()

const router = useRouter()
</script>

<template>
  <x-drawer v-model="navStore.leftDrawer" width="300">
    <template #header>
      <figure class="flex aspect-video flex-col items-stretch">
        <img src="/images/backgrounds/banner.jpg" alt="Banner" class="h-full w-full grow object-cover" />
        <figcaption
          class="line-clamp-1 flex shrink-0 items-center justify-center bg-primary-500 px-4 py-3 font-medium text-white">
          Ad
          majorem
          dei gloriam</figcaption>
      </figure>
    </template>
    <ul class="overflow-hidden">
      <li>
        <button type="button" class="flex w-full items-stretch justify-stretch" v-wave
          @click="async () => { await Browser.open({ url: 'https://theochrone.fr', toolbarColor: '#55acee' }); navStore.setLeftDrawer(false) }">
          <span class="aspect-square shrink-0 p-4">
            <IconCSS name="lets-icons:world-2-light" />
          </span>
          <span class="flex grow flex-col items-stretch justify-center p-2 text-left">
            <span class="line-clamp-1 font-medium text-secondary-800">Site internet</span>
            <span class="line-clamp-1 text-gray-500">https://theochrone.fr</span>
          </span>
        </button>
      </li>
      <li>
        <button type="button" class="flex w-full items-stretch justify-stretch" v-wave
          @click="router.push('/settings'); navStore.setLeftDrawer(false)">
          <span class="aspect-square shrink-0 p-4">
            <IconCSS name="lets-icons:setting-line-light" />
          </span>
          <span class="flex grow flex-col items-stretch justify-center p-2 text-left">
            <span class="line-clamp-1 font-medium text-secondary-800">Paramètres</span>
          </span>
        </button>
      </li>
      <li>
        <button type="button" class="flex w-full max-w-full items-stretch justify-stretch" v-wave
          @click="async () => { await Browser.open({ url: 'https://theochrone.fr/kalendarium/contact#contact', toolbarColor: '#55acee' }); navStore.setLeftDrawer(false) }">
          <span class="aspect-square shrink-0 p-4">
            <IconCSS name="lets-icons:edit-light" />
          </span>
          <span class="flex grow flex-col items-stretch justify-center overflow-hidden p-2 text-left">
            <span class="line-clamp-1 font-medium text-secondary-800">Signaler une erreur</span>
          </span>
        </button>
      </li>
      <li>
        <button type="button" class="flex w-full items-stretch justify-stretch" v-wave
          @click="async () => { App.exitApp() }">
          <span class="aspect-square shrink-0 p-4">
            <IconCSS name="lets-icons:sign-out-squre-light" />
          </span>
          <span class="flex grow flex-col items-stretch justify-center p-2 text-left">
            <span class="line-clamp-1 font-medium text-secondary-800">Quitter</span>
          </span>
        </button>
      </li>
    </ul>

    <template #footer>
      <div class="border-t p-4">
        <x-button color="pink" rounded block light
          @click="async () => { navStore.setLeftDrawer(false); await Browser.open({ url: 'https://www.paypal.com/donate/?hosted_button_id=38SVK696BLC8Y', toolbarColor: '#55acee' }) }">
          Faire un don
        </x-button>
      </div>
    </template>
  </x-drawer>
</template>
